...
const bsModalInstrument = new window.bootstrap.Modal(modalAddInstrumentEl);
const bsModalType = new window.bootstrap.Modal(modalAddTypeEl);
const bsModalUser = new window.bootstrap.Modal(modalAddUserEl);
...
async function showInstruments() {
    try {
        ...
        const select = document.getElementById("selectInstrumentType");
        if (select) {
            const types = await api.getTypes();
            select.innerHTML = types.map((t) => `<option value="${t._id}">${t.name}</option>`).join("");
        }
...
const formAddInstrument = document.getElementById("formAddInstrument");
